...
class NeuPrintUpdater:
    ...
    def _merge_segments(self, bodylist, properties, uuid, timestamp, txn):
        ...
        if "rootLocation" in mergedinfo:
            coords = mergedinfo["rootLocation"]["coordinates"]
            mergedinfo["rootLocation"] = f"POINT( {{ x: {coords[0]}, y: {coords[1]}, z: {coords[2]} }} )POINT"

        if "tosomaLocation" in mergedinfo:
            coords = mergedinfo["tosomaLocation"]["coordinates"]
            mergedinfo["tosomaLocation"] = f"POINT( {{ x: {coords[0]}, y: {coords[1]}, z: {coords[2]} }} )POINT"

        if "position" in mergedinfo:
            coords = mergedinfo["position"]["coordinates"]
            mergedinfo["position"] = f"POINT( {{ x: {coords[0]}, y: {coords[1]}, z: {coords[2]} }} )POINT"
    ...
    def split_segment(self, bodyid, synapse_list, properties2, properties1=None, uuid=None, timestamp=None, debug=False):
        ...
        with Transaction(self.full_dataset, client=self.client) as txn:
            ...
            # don't save split if in debug mode            
            if debug:
                txn.kill()
    ...
    def _split_segment(self, bodyid, synapse_list, properties2, properties1, uuid, timestamp, txn):
        ...
        if "rootLocation" in body2info:
            del body2info["rootLocation"]
        if "tosomaLocation" in body2info:
            del body2info["tosomaLocation"]
        if "position" in body2info:
            del body2info["position"]
        ...
        if "rootLocation" in body1info:
            coords = body1info["rootLocation"]["coordinates"]
            body1info["rootLocation"] = f"POINT( {{ x: {coords[0]}, y: {coords[1]}, z: {coords[2]} }} )POINT"
        if "tosomaLocation" in body1info:
            coords = body1info["tosomaLocation"]["coordinates"]
            body1info["tosomaLocation"] = f"POINT( {{ x: {coords[0]}, y: {coords[1]}, z: {coords[2]} }} )POINT"
        if "position" in body1info:
            coords = body1info["position"]["coordinates"]
            body1info["position"] = f"POINT( {{ x: {coords[0]}, y: {coords[1]}, z: {coords[2]} }} )POINT"
    ...
    def _update_segment_properties(self, bodyid, properties, uuid, timestamp, txn):
        ...
        if "rootLocation" in currprops:
            coords = currprops["rootLocation"]["coordinates"]
            currprops["rootLocation"] = f"POINT( {{ x: {coords[0]}, y: {coords[1]}, z: {coords[2]} }} )POINT"

        if "tosomaLocation" in currprops:
            coords = currprops["tosomaLocation"]["coordinates"]
            currprops["tosomaLocation"] = f"POINT( {{ x: {coords[0]}, y: {coords[1]}, z: {coords[2]} }} )POINT"

        if "position" in currprops:            
            print("Type:", type(currprops["position"]))
            if type(currprops["position"]) is dict:
                coords = currprops["position"]["coordinates"]
            elif type(currprops["position"]) is list:
                coords = currprops["position"]
            else:
                coords = currprops["position"]["coordinates"]
            currprops["position"] = f"POINT( {{ x: {coords[0]}, y: {coords[1]}, z: {coords[2]} }} )POINT"
