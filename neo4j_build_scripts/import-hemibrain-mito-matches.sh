...
#EXPORT_DIR=/mb6
EXPORT_BASE_DIR=/groups/flyem/data/neuprint_exports/syn_base_v5k
EXPORT_DIR=/neuprint_imports_csv_${UUID}_mito
#EXPORT_DIR=/neuprint_imports_csv_20631_v1.1

NEO4J_DIR=/opt/neo4j
IMPORT_BASE_DIR=/opt/neo4j/import

NEO4J_DB=${UUID}_nt.db
...
NEURON_MATCH_REL_CMD="--relationships:NblastMatchTo=import/Neuprint_Neuron_Matches_${UUID}.csv"
...
/var/lib/neo4j/bin/neo4j-admin import --database=${NEO4J_DB} ${META_NODE_CMD} ${NEURON_NODE_CMD} ${NEURON_REL_CMD} ${NEURON_MATCH_REL_CMD} ${SYNSET_NODE_CMD} ${SYNSET_REL_CMD} ${SYNAPSE_NODE_CMD} ${SYNAPSE_REL_CMD} ${MITO_NODE_CMD} ${MITOSET_NODE_CMD} ${MITO_REL_CMD} ${MITOSET_REL_CMD} ${MITOSYNSET_REL_CMD}
